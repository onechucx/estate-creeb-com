...
<header class="bg-brand-surface dark:bg-dark-surface p-4 border-b border-brand-border dark:border-dark-border flex items-center justify-between">
    ...
    <div class="flex items-center space-x-6">
        <div class="relative">
            <MagnifyingGlassIcon class="h-5 w-5 text-gray-400 dark:text-gray-500 absolute top-1/2 left-3 transform -translate-y-1/2" />
            <input
                type="text"
                placeholder="Search..."
            ...
            <button class="relative text-gray-500 hover:text-brand-primary dark:text-gray-400 dark:hover:text-dark-primary">
            <BellIcon class="h-6 w-6" />
            <span class="absolute top-0 right-0 h-2 w-2 bg-red-500 rounded-full" />
        </button>

        <div class="relative">
            <div class="flex items-center space-x-3 cursor-pointer" on:click={() => (isDropdownOpen = !isDropdownOpen)}>
            ...
            {#if isDropdownOpen}
                <div class="absolute right-0 mt-2 w-48 bg-brand-surface dark:bg-dark-surface rounded-md shadow-lg py-1 z-50 border border-brand-border dark:border-dark-border">
                    <div class="px-4 py-2">
                        <label class="text-xs text-gray-500 dark:text-dark-text-secondary">Switch Role</label>
                        <select bind:value={role} on:change={() => setUserRole(role)} class="w-full text-sm bg-transparent focus:outline-none dark:text-dark-text-primary">
                    ...
                    <button on:click={onLogout} class="w-full text-left px-4 py-2 text-sm text-brand-text-primary dark:text-dark-text-primary hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center">
