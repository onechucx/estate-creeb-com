<script lang="ts">
    ...
    // Keep a single timer that resets whenever `message` changes.
    ...
    $: {
        ...
        if (message) {
            timer = setTimeout(() => dispatch('dismiss'), 5000);
        }
    }
...
{#if message}
    <div class="fixed top-5 right-5 bg-brand-surface dark:bg-dark-surface shadow-lg rounded-lg p-4 flex items-center z-[100] animate-fade-in-down border border-brand-border dark:border-dark-border" role="status" aria-live="polite">
        <svelte:component this={iconMap[message.type] ?? InformationCircleIcon}
        ...
        <button type="button" aria-label="Dismiss toast" title="Dismiss toast" on:click={() => dispatch('dismiss')} class="ml-4 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">&times;</button>
    </div>
{/if}
