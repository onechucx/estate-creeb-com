...
<aside class="w-64 bg-brand-surface dark:bg-dark-surface flex-shrink-0 p-4 border-r border-brand-border dark:border-dark-border flex flex-col">
    <div class="flex items-center mb-10 p-2">
        <svg class="h-10 w-10 text-brand-primary dark:text-dark-primary" viewBox="0 0 24 24" fill="currentColor">
    ...
    <nav class="flex-1">
        <ul class="space-y-1">
            {#each visibleNavItems as item}
                ...
                <li on:click={() => setActiveView(item.view)} class={itemClass(isActive)}>
        ...
        {#if visibleActionNavItems.length > 0}
            ...
            <ul class="space-y-1">
                {#each visibleActionNavItems as item}
                    ...
                    <li on:click={!isDisabled ? () => setActiveView(item.view) : undefined} class={actionItemClass(isDisabled)} title={isDisabled ? 'An active subscription is required for this feature.' : ''}>
                        <svelte:component this={item.icon} class="h-6 w-6 mr-4" />
                        <span class="font-medium">{item.label}</span>
                    </li>
                {/each}
